...
def get_canny(image, low_threshold=100, high_threshold=200):
    ...
    canny = cv2.Canny(canny, low_threshold, high_threshold)[..., None]
    canny = img2tensor(canny).unsqueeze(0) / 255.
    return canny


def get_color(image, resize_factor=64):
    color = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
    color = resize_numpy_image(color)
    H, W = color.shape[:2]

    color = cv2.resize(color, (W // resize_factor, H // resize_factor), interpolation=cv2.INTER_CUBIC)
