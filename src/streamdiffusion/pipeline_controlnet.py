...
class StreamControlNetDiffusion:
    def __init__(
        self,
    ...
    def prepare(
        ...
        #self.timesteps, self.num_inference_steps = self.get_timesteps(self.num_inference_steps, strength, self.device)
    ...
    def predict_x0_batch(
        ...
        if self.use_denoising_batch:
            ...
            x_0_pred_batch, model_pred = self.unet_step(
                x_t_latent, t_list, image, controlnet_conditioning_scale
            )
            ...
            for idx, t in enumerate(self.sub_timesteps_tensor):
                ...
                x_0_pred, model_pred = self.unet_step(
                    x_t_latent, t, image, controlnet_conditioning_scale, idx
                )
    ...
    def __call__(
        self, 
        x: Union[torch.Tensor, PIL.Image.Image, np.ndarray],
        control_x: Union[torch.Tensor, PIL.Image.Image, np.ndarray],
        controlnet_conditioning_scale: Optional[float] = 1.0,
    ) -> torch.Tensor:
        ...
        x = self.pipe.image_processor.preprocess(x, self.height, self.width).to(
