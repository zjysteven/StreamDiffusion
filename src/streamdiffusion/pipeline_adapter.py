...
from diffusers import (
    ...
    MultiAdapterWithFuser, MultiAdapter
)
...
class StreamAdapterDiffusion:
    ...
    def predict_x0_batch(
        ...
        if self.use_denoising_batch:
            ...
            x_0_pred_batch, model_pred = self.unet_step(
                x_t_latent, t_list, adapter_state
            )
            ...
            for idx, t in enumerate(self.sub_timesteps_tensor):
                ...
                x_0_pred, model_pred = self.unet_step(
                    x_t_latent, t, adapter_state, idx
                )
    ...
    def __call__(
        ...
        # sanity check
        if isinstance(self.adapter, (MultiAdapter, MultiAdapterWithFuser)):
            ...
            if not isinstance(adapter_conditioning_scale, list) or len(adapter_conditioning_scale) != len(self.adapter.adapters):
                raise ValueError(
                    "MultiAdapter is enabled, but `adapter_conditioning_scale` is not a list. Please pass a list of conditioning scales to `adapter_conditioning_scale`."
                )
        
        # preprocess cond image(s)
        if isinstance(self.adapter, (MultiAdapter, MultiAdapterWithFuser)):
            adapter_input = []

            for one_image in image:
                one_image = _preprocess_adapter_image(one_image, self.height, self.width)
                one_image = one_image.to(device=self.device, dtype=self.adapter.dtype)
                adapter_input.append(one_image)
        else:
            adapter_input = _preprocess_adapter_image(image, self.height, self.width)
            adapter_input = adapter_input.to(device=self.device, dtype=self.adapter.dtype)

        # get adapter_state
        if isinstance(self.adapter, (MultiAdapter, MultiAdapterWithFuser)):
            adapter_state = self.adapter(adapter_input, adapter_conditioning_scale)
            for k, v in enumerate(adapter_state):
                adapter_state[k] = v.type(self.unet.dtype)
        else:
            adapter_state = self.adapter(adapter_input)
            for k, v in enumerate(adapter_state):
                adapter_state[k] = (v * adapter_conditioning_scale).type(self.unet.dtype)
        
        for k, v in enumerate(adapter_state):
            adapter_state[k] = v.repeat(self.batch_size, 1, 1, 1)
        ...
        x_t_latent = torch.randn((1, 4, self.latent_height, self.latent_width), generator=generator).to(
            device=self.device, dtype=self.dtype
        )
        x_0_pred_out = self.predict_x0_batch(x_t_latent, adapter_state)
